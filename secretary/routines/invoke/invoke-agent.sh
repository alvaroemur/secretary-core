#!/usr/bin/env bash
# Invoca agent CLI con logging, stream legible y métricas (tokens + costo estimado).
# Uso: invoke-agent.sh <routine_id> <workspace> <prompt>
set -euo pipefail
...
# Symlinks "latest" para tail en vivo sin buscar timestamp
...
set +e
cd "$WORKSPACE"
agent -p --trust --force --model "$AGENT_MODEL" \
